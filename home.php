...
<div class="realtime-agent-info">
	<div id="wrapper">
		...
		<?php			
			...
			mysql_query("DELETE FROM `chat`.`internal_conversation_notification` WHERE send_by = '".$received_by."' AND received_by = '".$received_by."'");
			$result_msglist_count = mysql_query("SELECT count(*) as totmsg FROM `chat`.`internal_conversation` WHERE send_by IN ('".$received_by."','".$send_by."') and received_by IN ('".$received_by."','".$send_by."')");			
			$row_count = mysql_fetch_assoc($result_msglist_count);
			...
			$result_msglist = mysql_query("SELECT * FROM `chat`.`internal_conversation` WHERE send_by IN ('".$received_by."','".$send_by."') and received_by IN ('".$received_by."','".$send_by."') ORDER BY `id` ASC LIMIT $total_message, 50");			
