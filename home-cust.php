...
<div class="realtime-agent-info">
	<div id="wrapper">
		...
		<?php			
			...
			$check_agent = mysql_query("SELECT * FROM `chat`.`chat_id` WHERE id = '".$chat_id."' AND `agent_name` = '".$send_by."'");
			if(mysql_num_rows($check_agent)>0){
				mysql_query("DELETE FROM `chat`.`chat_customer_notification` WHERE chat_id = '".$chat_id."'");
				$result_msglist_count = mysql_query("SELECT count(*) as totmsg FROM `chat`.`chat_customer` WHERE chat_id = '".$chat_id."'");			
				...
				$result_msglist = mysql_query("SELECT * FROM `chat`.`chat_customer` WHERE chat_id = '".$chat_id."' ORDER BY `id` ASC LIMIT $total_message, 50");			
